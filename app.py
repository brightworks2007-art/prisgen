...
with app.app_context():
    ...
    try:
        ...
        # Product table — add stock + limited_stock if missing
        ...
        with db.engine.connect() as conn:
            if 'stock' not in existing:
                ...
                conn.commit()
            if 'limited_stock' not in existing:
                conn.execute(text('ALTER TABLE product ADD COLUMN limited_stock BOOLEAN DEFAULT FALSE'))
                conn.commit()

        # AbandonedCart table — create_all handles new tables, but ensure it exists
        db.create_all()
